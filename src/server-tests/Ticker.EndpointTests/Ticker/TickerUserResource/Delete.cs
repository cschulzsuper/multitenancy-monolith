...
public sealed class Delete : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task Delete_ShouldFail_WhenAbsent()
    {
        ...
        var request = new HttpRequestMessage(HttpMethod.Delete, $"/api/b1/ticker/ticker-users/{absentTickerUser}");
        request.Headers.Authorization = _factory.MockValidMemberAuthorizationHeader();
    ...
    public async Task Delete_ShouldFail_WhenInvalid()
    {
        ...
        var request = new HttpRequestMessage(HttpMethod.Delete, $"/api/b1/ticker/ticker-users/{invalidTickerUser}");
        request.Headers.Authorization = _factory.MockValidMemberAuthorizationHeader();

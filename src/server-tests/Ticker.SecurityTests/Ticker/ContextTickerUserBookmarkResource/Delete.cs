...
public sealed class Delete : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task Delete_ShouldBeUnauthorized_WhenNotAuthenticated()
    {
        ...
        var request = new HttpRequestMessage(HttpMethod.Delete, $"/api/ticker/ticker-users/me/bookmarks/{validBookmark}");
    ...
    public async Task Delete_ShouldBeUnauthorized_WhenInvalid(int mock)
    {
        ...
        var request = new HttpRequestMessage(HttpMethod.Delete, $"/api/ticker/ticker-users/me/bookmarks/{validTickerMessage}");

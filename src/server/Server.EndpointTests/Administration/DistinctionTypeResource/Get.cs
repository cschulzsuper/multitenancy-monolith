...
public sealed class Get : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task Get_ShouldSucceed_WhenExists(string identity, string group, string member)
    {
        ...
        Assert.Collection(content.OrderBy(x => x.Key),
            x => Assert.Equal((x.Key, (string?) x.Value), ("displayName", existingDistinctionType.DisplayName)),
            x => Assert.Equal((x.Key, (string?) x.Value), ("objectType", existingDistinctionType.ObjectType)),
            x => Assert.Equal((x.Key, (string?) x.Value), ("uniqueName", existingDistinctionType.UniqueName)));
    }

...
public sealed class Post : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task Post_ShouldFail_WhenValidMember()
    {
        // Arrange
        var request = new HttpRequestMessage(HttpMethod.Post, "/api/ticker/ticker-messages");
        request.Headers.Authorization = _factory.MockValidAuthorizationHeader(MockWebApplication.MockMember);
        request.Content = JsonContent.Create(new object());
    ...
    [Fact]
    public async Task Post_ShouldBeForbidden_WhenUnauthorized()
    {
        // Arrange
        var request = new HttpRequestMessage(HttpMethod.Post, "/api/ticker/ticker-messages");
        request.Headers.Authorization = _factory.MockValidAuthorizationHeader(MockWebApplication.MockTicker);
        request.Content = JsonContent.Create(new object());

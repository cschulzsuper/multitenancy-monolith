...
public static class TickerUserDependentCommands
{
    ...
    public static IEndpointRouteBuilder MapTickerUserDependentCommands(this IEndpointRouteBuilder endpoints)
    {
    ...
    private static Delegate Auth =>
        (ITickerUserDependentCommandHandler commandHandler, TickerUserDependentAuthCommand command)
            => commandHandler.AuthAsync(command);

    private static Delegate Confirm =>
        (ITickerUserDependentCommandHandler commandHandler, TickerUserDependentConfirmCommand command)
            => commandHandler.ConfirmAsync(command);

    private static Delegate Post =>
        (ITickerUserDependentCommandHandler commandHandler, TickerUserDependentPostCommand command)
            => commandHandler.PostAsync(command);

    private static Delegate Verify =>
        (ITickerUserDependentCommandHandler commandHandler)
